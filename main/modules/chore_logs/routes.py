...
from .models import ChoreLogViewModel
...
@min_clearance(ClearanceEnum.NORMAL)
def index():
    chore_logs_list = [ChoreLogViewModel(chore_log) for chore_log in services.generate_next_chore_logs()]
...
def complete():
    ...
    updated_chore_log_view_model = ChoreLogViewModel(updated_chore_log)
    return render_template("chore_logs/chore-log-partial.html",
                           chore_log=updated_chore_log_view_model)
...
def undo(chore_log_id: int):
    ...
    replace_chore_log_view_model = ChoreLogViewModel(replace_chore_log)
    return render_template("chore_logs/chore-log-partial.html",
                           chore_log=replace_chore_log_view_model)
